...
lob::strategies::AvellanedaStoikovStrategyConfig avellaneda_strategy_config() {
...
TEST(BacktestEngineIntegrationTest, AvellanedaStoikovSampleRunWritesFiniteMetricsAndArtifacts) {
  const auto data_dir = std::filesystem::path(LOB_TEST_DATA_DIR);
  ASSERT_TRUE(std::filesystem::exists(data_dir / "lob.csv"));
  ASSERT_TRUE(std::filesystem::exists(data_dir / "trades.csv"));
